# Phase 24B — MoE Design Safety Check & Review
...
```
    ...
    [Softmax Weights w]     [Expert Heights H1, H2, H3]
         └──────────┬──────────┘
                    ↓
          [Weighted Prediction]
            H = Σ(w_i * H_i)                                      <-- Fully Differentiable
```

---

## 4. Gating & Anti-Collapse Formulation
...
1. **Auxiliary Gate Supervision:**
   During training, we apply a multi-class Cross Entropy loss on the gate routing weights $w$ against the true regime labels:
   $$\mathcal{L}_{gate\_aux} = \text{CrossEntropy}(w, \text{true\_regime})$$
   *This acts as a teacher signal forcing the gate to active Mid and High structures to their respective experts.*
...
2. **Gating Balance Regularization:**
   ...
   $$\mathcal{L}_{balance} = \sum_{i=1}^3 \left( \bar{w}_i - \frac{1}{3} \right)^2$$
   where $\bar{w}_i$ is the average gating probability of expert $i$ over the mini-batch.
...
- $M_1(H) = \exp(-(H - 5)^2 / 50)$ (peaks at 5m, active 0-15m)
- $M_2(H) = \exp(-(H - 20)^2 / 100)$ (peaks at 20m, active 10-35m)
- $M_3(H) = \exp(-(H - 50)^2 / 400)$ (peaks at 50m, active 30m+)
...
## 6. Success & Invalidation Criteria

### Success Criteria:
The MoE model is successful only if:
1. **Skyscraper (>40m) MAE** on New York zero-shot is reduced to **$< 25.0\text{m}$**.
2. **Skyscraper (>40m) Bias** is reduced to **$> -25.0\text{m}$**.
3. **Low-Rise (<10m) MAE** on Copenhagen/New York remains **$< 4.5\text{m}$**.
4. **Overall Building MAE** on New York is **$< 15.0\text{m}$**.

### Invalidation Criteria:
We reject the design and abandon the MoE approach if:
- Gating weights collapse ($w_1 \approx 1$ for all inputs).
- $E_3$ predictions remain near the training mean ($5\text{m} - 15\text{m}$) on New York skyscrapers.
- Predictions exhibit widespread unstable spikes ($>150\text{m}$).
