...
OUT_DIR = Path("runs/phase19_building_conditioned_design")
OUT_DIR.mkdir(parents=True, exist_ok=True)

design_md = """# PHASE 19A — BUILDING-CONDITIONED HEIGHT MODEL DESIGN
...
- **Topography Preservation:** To preserve intra-building roof topology, we extract the relative-depth patch $N_{rel, i}$ inside component $i$, min-max normalize it locally to $[0, 1]$, and multiply it by the predicted building scale $S_i$:
  $$h_{pixel} = (S_i - 2.0) \times \frac{N_{rel, i} - \min(N_{rel, i})}{\max(N_{rel, i}) - \min(N_{rel, i}) + \epsilon} + 2.0$$
...
results_json = {
  "phase18_evidence": {
    "tile_level_mae": 47.99,
    "building_level_predicted_p95_mae": 28.84,
    "building_level_oracle_p95_mae": 26.94,
    "building_level_pearson_r": 0.312
  },
  "recommended_architecture": "Candidate B (OOTS-Net)",
  "smallest_falsifiable_experiment": {
    "train_set": "128 tiles DFC2023",
    "test_set": "New York zero-shot",
    "falsification_threshold_mae": 32.0,
    "falsification_threshold_pearson": 0.20
  },
  "final_action": "PROCEED TO IMPLEMENTATION"
}
...
print("Saved DESIGN.md and results.json to runs/phase19_building_conditioned_design/")
