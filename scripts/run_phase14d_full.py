...
OUT_DIR = Path("runs/phase14d_correct_decoder_adapt")
...
def main():
    ...
    test_samples = load_data_in_memory(test_tids, load_cls=True) # Load CLS for test metrics
    ...
    for seed in [1]:
        ...
        for i, s in enumerate(test_samples):
            pred = model.predict_full(s['rgb'], s['gt'].shape[:2], device)
            
            # metrics
            cls_mask = s.get('cls', None)
            metrics = compute_class_metrics(pred, s['gt'], cls_mask, building_label=6, nodata=-999.0)
            binned = compute_binned_metrics(pred, s['gt'], edges=bin_edges, nodata=-999.0)
            
            per_scene.append(metrics)
            binned_scenes.append(binned)
            
            if "NewYork" in s["id"] and s["gt"].max() > 30 and i < 20:
                valid = (np.isfinite(s["gt"])) & (s["gt"] != -999.0)
                err = np.zeros_like(pred)
                err[valid] = pred[valid] - s["gt"][valid]
                err[~valid] = 0.0
                render_comparison(s['rgb'], s['gt'], pred, err, seed_dir / f"figures/vis_{s['id'].replace('.tif', '')}.png")
