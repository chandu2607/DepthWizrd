"""
Phase 33C — Final 3D City Polish Script.
...
OUT_DIR = Path("runs/phase33c_final_3d_polish")
...
print("=== PHASE 33C — FINAL 3D CITY POLISH AUDIT ===")
...
def build_polished_city_meshes(Z_dsm, Z_dtm, mask_bldg, transform, min_area=15, exaggeration=1.0):
    """Phase 33C: Builds separate surface (terrain+roof) and side-wall PolyData objects
...
def render_polished_scene(mesh_surf, mesh_walls, camera, render_mode="RGB City"):
    ...
    if render_mode in ["RGB City", "RGB Texture"]:
        ...
        pl.add_mesh(mesh_surf, scalars='Elevation', cmap="plasma", show_edges=False, smooth_shading=True, ambient=0.3, diffuse=0.85)
        if mesh_walls is not None:
            pl.add_mesh(mesh_walls, scalars='Elevation', cmap="plasma", show_edges=False, smooth_shading=False, ambient=0.25, diffuse=0.8)
        pl.add_scalar_bar("Elevation (m)", title_font_size=14)
        
    elif render_mode == "Building Height Structure":
        pl.add_mesh(mesh_surf, scalars='BuildingHeight', cmap="viridis", show_edges=False, smooth_shading=True, ambient=0.3, diffuse=0.85)
        if mesh_walls is not None:
            pl.add_mesh(mesh_walls, scalars='BuildingHeight', cmap="viridis", show_edges=False, smooth_shading=False, ambient=0.25, diffuse=0.8)
...
axes[1].imshow(img_after);  axes[1].set_title("AFTER: Phase 33C Polished 3D City (1:1 RGB Roofs + Slate Wall Material)", fontsize=13, fontweight="bold")
for ax in axes: ax.axis("off")
plt.suptitle("Phase 33C — Final 3D City Polish Audit\n"
...
report = f"""# Phase 33C — Final 3D City Polish Report
...
print("\nPhase 33C Final 3D City Polish script completed successfully.")
